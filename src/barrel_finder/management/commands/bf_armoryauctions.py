...
class Command(BaseCommand):
    def handle(self, *args, **options):
        portal = get_portal('armoryauctions')

        # time.sleep(1)
        for page in range(1, 2):
            page_url = f'https://armory-auctions.pl/ogloszenia/bron-i-amunicja?created=&location=&page={page}&price_from=&price_to=&search=&sort=&type='
            print("Page ", page, page_url)
            response = requests.get(page_url)

            if response.status_code != 200:
                print('Error getting response from ArmoryAuctions', response.status_code, response.text)
                return
